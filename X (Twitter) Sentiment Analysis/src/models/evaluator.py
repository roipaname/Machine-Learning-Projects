...
class ModelEvaluator:
    ...
    def plot_learning_curves(
        ...
        try:
            ...
            ax.plot(train_sizes_abs, val_mean, 'o-', color='g', label='Validation score', linewidth=2)
            ax.fill_between(
                train_sizes_abs,
                val_mean - val_std,
                val_mean + val_std,
    ...
    def plot_precision_recall_by_class(
        ...
        try:
            ...
            bars1 = ax1.bar(x_pos, precision, color='skyblue', alpha=0.8, edgecolor='navy')
            ...
            # Add value labels
            for i, (bar, v) in enumerate(zip(bars1, precision)):
                ax1.text(bar.get_x() + bar.get_width()/2, v + 0.02, 
                        f'{v:.3f}', ha='center', va='bottom', fontsize=9)
            
            # Plot recall
            bars2 = ax2.bar(x_pos, recall, color='lightcoral', alpha=0.8, edgecolor='darkred')
            ...
            # Add value labels
            for i, (bar, v) in enumerate(zip(bars2, recall)):
                ax2.text(bar.get_x() + bar.get_width()/2, v + 0.02, 
                        f'{v:.3f}', ha='center', va='bottom', fontsize=9)
            
            fig.suptitle(f'{model_name} - Per-Class Metrics', fontsize=16, fontweight='bold', y=1.00)
            plt.tight_layout()
            ...
            logger.error(f"Error plotting precision/recall: {e}")
            plt.close('all')
            return None
        ...
        try:
            ...
            for model_name in model_names:
                if 'metrics' in results[model_name]:
                    ...
                    metrics_dict['Accuracy'].append(m.get('accuracy', 0))
                    metrics_dict['Precision'].append(m.get('precision', 0))
                    metrics_dict['Recall'].append(m.get('recall', 0))
                    metrics_dict['F1 Score'].append(m.get('f1_score', 0))
                else:
                    # Model failed
    ...
    def _calculate_roc_auc(
        ...
        try:
            ...
            valid_scores = [v for v in roc_auc.values() if v > 0]
            if valid_scores:
                roc_auc['macro_avg'] = float(np.mean(valid_scores))
            else:
                roc_auc['macro_avg'] = 0.0
            
            return roc_auc
            
        except Exception as e:
            logger.error(f"Error calculating ROC AUC: {e}")
            return {'macro_avg': 0.0}
