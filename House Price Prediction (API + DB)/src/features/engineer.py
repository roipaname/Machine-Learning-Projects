...
class FeatureEngineer:
    ...
    def create_numerical_features(self,df:pd.DataFrame)->pd.DataFrame:
        ...
        features['has_prev-sale']=df['prev_sold_date'].notna().astype(int)
        features['is_large_house']=(df['house_size']>df['house_size'].median()).astype(int)
    ...
    def engineer_features(self)->pd.DataFrame:
        ...
        print(f"  Total features created: {len(features.columns) - 1}")  # -1 for property_id
        print(f"  Feature names: {', '.join(features.columns[1:])}")
    ...
    def get_feature_statistics(self):
        ...
        SELECT 
            ...
            SUM(CASE WHEN has_prev_sale = 1 THEN 1 ELSE 0 END) as properties_with_prev_sale,
            SUM(CASE WHEN is_large_house = 1 THEN 1 ELSE 0 END) as large_houses
